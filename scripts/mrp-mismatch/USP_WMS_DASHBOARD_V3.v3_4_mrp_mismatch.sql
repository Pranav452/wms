...
/*
  USP_WMS_DASHBOARD_V3  -  WMS Stock-Status Dashboard (26 result sets)
  v3.4 (MRP mismatch, mail 17/07/2026):
    - STEP 11 #MRP_MM: PO MRP vs WMS label MRP mismatches per PONO+EAN
      (same rule as FLAG=0 in USP_IMP_WMS_GET_CLIENTPO_DOWNLOADEXCEL)
    - RS12 + RS22 gain MRP_MISMATCH_COUNT
    - RS21 gains MRP_MISMATCH_EAN_COUNT / MRP_MISMATCH_PO_COUNT
    - RS26 MRP MISMATCH EAN DETAIL appended
    - backup of the previous version: USP_WMS_DASHBOARD_V3_BKUP_20260717
  ...
  v3.2 (KIABI mail 09/07/2026):
    ...
    - backup of the previous version: USP_WMS_DASHBOARD_V3_BKUP_20260710
  Fixes (v3.1):
...
BEGIN
    ...
    /* ════════════════════════════════════════════════════════════
       STEP 11 : MRP MISMATCH — PO MRP vs WMS LABEL MRP
       Same rule as FLAG=0 in USP_IMP_WMS_GET_CLIENTPO_DOWNLOADEXCEL:
       PO MRP string NOT found inside comma-joined label prices.
    ════════════════════════════════════════════════════════════ */

    -- 11a: one row per PONO+EAN with deduplicated PO MRP / article code
    IF OBJECT_ID('tempdb..#PO_EAN') IS NOT NULL DROP TABLE #PO_EAN;
    SELECT
        PONO,
        EAN             = GTIN,
        VENDOR_ART_CODE = MAX(VENDORARTNO),
        SKU             = MAX(SKU),
        PO_MRP          = MAX(MRP)
    INTO #PO_EAN
    FROM TBL_WMS_CLIENTPO
    WHERE PONO NOT IN ('DEADSTOCK260220-00')
    GROUP BY PONO, GTIN;
    CREATE NONCLUSTERED INDEX IX_POEAN_EAN ON #PO_EAN (EAN);

    -- 11b: attach label MRPs and current balance, then keep only mismatches
    IF OBJECT_ID('tempdb..#MRP_MM') IS NOT NULL DROP TABLE #MRP_MM;
    SELECT
        P.PONO,
        P.EAN,
        P.VENDOR_ART_CODE,
        P.SKU,
        P.PO_MRP,
        LBL_MRP = ISNULL(STUFF((
                      SELECT ',' + CONVERT(VARCHAR(20), LBL.PRICE)
                      FROM WMS_ITEM_MASTER_MRPLABEL LBL
                      WHERE LBL.EAN = P.EAN
                      FOR XML PATH(''), TYPE
                  ).value('.','NVARCHAR(MAX)'), 1, 1, ''), ''),
        BAL_QTY = ISNULL(S.BAL_QTY, 0)
    INTO #MRP_MM
    FROM #PO_EAN P
    LEFT JOIN #STOCK S ON S.EAN = P.EAN;

    -- delete rows where PO MRP IS found in label MRPs (keep FLAG=0 rows only)
    DELETE FROM #MRP_MM
    WHERE CHARINDEX(CONVERT(VARCHAR(20), PO_MRP), LBL_MRP) > 0;

    CREATE NONCLUSTERED INDEX IX_MRPMM_PONO ON #MRP_MM (PONO);

    IF OBJECT_ID('tempdb..#PO_EAN') IS NOT NULL DROP TABLE #PO_EAN; -- no longer needed
    ...
    /* ════════════════════════════════════════════════════════════
       ...
       v3.4: + MRP_MISMATCH_COUNT (EANs on the PO failing the label check)
    ════════════════════════════════════════════════════════════ */
    SELECT
        PO.PONO,
        PO.CLIENT,
        ...
        MRP_MISMATCH_COUNT = ISNULL((
            SELECT COUNT(*)
            FROM #MRP_MM MM
            WHERE MM.PONO = PO.PONO
        ), 0),
    ...
    /* ════════════════════════════════════════════════════════════
       ...
       v3.4: + MRP_MISMATCH_EAN_COUNT / MRP_MISMATCH_PO_COUNT
    ════════════════════════════════════════════════════════════ */
    SELECT
        ...
        MRP_MISMATCH_EAN_COUNT = (SELECT COUNT(DISTINCT EAN) FROM #MRP_MM),
        MRP_MISMATCH_PO_COUNT  = (SELECT COUNT(DISTINCT PONO) FROM #MRP_MM);

    /* ════════════════════════════════════════════════════════════
       RS22 : STR TRACKING (PO level + STR date + dispatch TAT)
       In-window = STR created in window OR dispatched in window;
       no range -> every PO.
       v3.4: + MRP_MISMATCH_COUNT
    ════════════════════════════════════════════════════════════ */
    SELECT
        P.PONO,
        P.CLIENT,
        ...
        MRP_MISMATCH_COUNT = ISNULL((SELECT COUNT(*) FROM #MRP_MM MM WHERE MM.PONO = P.PONO), 0),
    ...
    /* ════════════════════════════════════════════════════════════
       RS26 : MRP MISMATCH EAN DETAIL
       PO MRP <> WMS label MRP — one row per PONO + EAN mismatch
    ════════════════════════════════════════════════════════════ */
    SELECT
        MM.PONO                     AS [PONO],
        MM.VENDOR_ART_CODE          AS [Vendor Article Code],
        MM.SKU                      AS [SKU],
        MM.EAN                      AS [EAN],
        ISNULL(IM.ITEMNAME, '')     AS [Item Name],
        MM.PO_MRP                   AS [PO MRP],
        MM.LBL_MRP                  AS [WMS Label MRP],
        MM.BAL_QTY                  AS [Balance Qty]
    FROM #MRP_MM MM
    LEFT JOIN WMS_ITEM_MASTER IM ON IM.EAN = MM.EAN
    ORDER BY MM.PONO, MM.EAN;
