...
const STEPS = [
  ...
  // v4 — email addresses + self-service password reset by email
  `IF COL_LENGTH('dbo.TBL_WMS_AUTH_USERS', 'EMAIL') IS NULL
   BEGIN
     ALTER TABLE dbo.TBL_WMS_AUTH_USERS ADD EMAIL VARCHAR(254) NULL;
     PRINT 'Added EMAIL';
   END`,
  // unique among accounts that have one; case-insensitive through the DB collation
  `IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'UX_TBL_WMS_AUTH_USERS_EMAIL' AND object_id = OBJECT_ID('dbo.TBL_WMS_AUTH_USERS'))
   BEGIN
     CREATE UNIQUE INDEX UX_TBL_WMS_AUTH_USERS_EMAIL ON dbo.TBL_WMS_AUTH_USERS (EMAIL) WHERE EMAIL IS NOT NULL;
     PRINT 'Added unique index on EMAIL';
   END`,
  `IF OBJECT_ID('dbo.TBL_WMS_AUTH_PASSWORD_RESETS', 'U') IS NULL
   BEGIN
     CREATE TABLE dbo.TBL_WMS_AUTH_PASSWORD_RESETS (
       ID         INT IDENTITY(1,1) NOT NULL CONSTRAINT PK_TBL_WMS_AUTH_PASSWORD_RESETS PRIMARY KEY,
       USER_ID    INT         NOT NULL,
       TOKEN_HASH CHAR(64)    NOT NULL CONSTRAINT UQ_TBL_WMS_AUTH_PASSWORD_RESETS_TOKEN UNIQUE,  -- SHA-256 of the emailed token; the token itself is never stored
       CREATED_AT DATETIME    NOT NULL CONSTRAINT DF_TBL_WMS_AUTH_PASSWORD_RESETS_CREATED DEFAULT (GETDATE()),
       EXPIRES_AT DATETIME    NOT NULL,
       USED_AT    DATETIME    NULL,
       REQUEST_IP VARCHAR(45) NULL
     );
     PRINT 'Created dbo.TBL_WMS_AUTH_PASSWORD_RESETS';
   END`,
  `IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'IX_TBL_WMS_AUTH_PASSWORD_RESETS_USER' AND object_id = OBJECT_ID('dbo.TBL_WMS_AUTH_PASSWORD_RESETS'))
   BEGIN
     CREATE INDEX IX_TBL_WMS_AUTH_PASSWORD_RESETS_USER ON dbo.TBL_WMS_AUTH_PASSWORD_RESETS (USER_ID, CREATED_AT);
     PRINT 'Added index on reset USER_ID';
   END`,
];
