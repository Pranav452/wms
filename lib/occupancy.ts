...
// ── per-drawer (single location) demo fill ───────────────────────────────────
export interface SlotStat { pct: number; occupied: boolean; items: number }
export function slotStat(code: string): SlotStat {
  const pct = span(`${code}|slot`, 0, 100)
  return { pct, occupied: pct > 12, items: span(`${code}|itm`, 0, 14) }
}

// ── whole-floor rollup for the metrics strip ─────────────────────────────────
export interface FloorRollup {
  capacity: number      // real: total locations on the floor
  occupancyPct: number
  volumeUsed: number
  volumeTotal: number
  items: number
...
export function floorRollup(floor: FloorId): FloorRollup {
  const rs = racksOnFloor(floor)
  const stats = rs.map(rackStat)
  return {
    capacity:     rs.reduce((s, r) => s + r.locations, 0),
    occupancyPct: Math.round(stats.reduce((s, r) => s + r.pct, 0) / stats.length),
    volumeUsed:   stats.reduce((s, r) => s + r.volumeUsed, 0),
    volumeTotal:  stats.reduce((s, r) => s + r.volumeTotal, 0),
    items:        stats.reduce((s, r) => s + r.itemCount, 0),
    nearFull:     stats.filter(r => r.pct > 85).length,
  }
}
